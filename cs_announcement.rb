...
stop_index = 0
stop_link = nil
filePath = 'cs.hit.edu.cn/announcement/last_update.json'
...
while !should_stop do
	table = news_index.search('//*[@id="block-system-main"]/div/div/div/div[1]/table[1]/tbody/tr') # use XPath query to get news_links
	table.each do |cell|
		news_title = cell.search('td//a').text.strip
		news_link = base_url + cell.search('td//a')[0]['href'] 
		news_date = cell.search('td[3]').text.strip
		if stop_link == nil
			stop_link = news_link
		end
		...
		parser = ACHTMLNodeParser.new(doc.xpath('//*[@id="'+cell.search('td//a')[0]["href"][4...8]+'-'+cell.search('td//a')[0]["href"][9...13]+'"]/div[2]/div[1]/div/div'), base_url)
		parser.parse
		news_date = news_date +' '+ DateTime.now.to_time.to_s.split(' ')[1] +' ' + DateTime.now.to_time.to_s.split(' ')[1]
		...
		news = NewsObject.new(HNDepart::CS, HNType::ANNOUNCEMENT, obj)
		news.save
		stop_index += 1
	end
